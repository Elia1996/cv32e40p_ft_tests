...
def GetModuleInfo(filename, modinfo_noft=0):
    ####################################################################################
    ...
    for line in fp.readlines():
        ...
        if not save_intern_signals:
            ...
            if " input " in line:
                if modinfo_noft!=0 and not real_line.split()[-1] in modinfo_noft["sig_input_name"]:
                    sig_input_name_ft.append(real_line.split()[-1])
                    sig_input_bits_ft.append(GetBits(line))
                else:
                    sig_input_name.append(real_line.split()[-1])
                    sig_input_bits.append(GetBits(line))
               
            if " output " in line:
                if modinfo_noft!=0 and not real_line.split()[-1] in modinfo_noft["sig_output_name"]:
                    sig_output_name_ft.append(real_line.split()[-1])
                    sig_output_bits_ft.append(GetBits(line))
                else:
    ...
    return {"module":module,"parameter_value":parameter_value, "parameter_name":parameter_name, "parameter_bits":parameter_bits, "sig_input_name" : sig_input_name , "sig_input_bits":sig_input_bits, "sig_output_name":sig_output_name, "sig_output_bits":sig_output_bits , "sig_intern_name": sig_intern_name, "sig_intern_bits":sig_intern_bits, "sig_input_name_ft":sig_input_name_ft, "sig_input_bits_ft":sig_input_bits_ft, "sig_output_name_ft":sig_output_name_ft, "sig_output_bits_ft":sig_output_bits_ft}
...
def getTripleIoPort(start,sig_name,sig_bits,append):
    ...
    ####################################################################################
    if (type(sig_bits) == list and int(sig_bits[0]) == 1) or str(sig_bits) == "1" :
...
def GetFTModuleIoDeclaration( module_info, ft_sig_info, indent, ft=1):
    ...
    if "sig_intern_name" in module_info.keys():
        for sig,bit in zip(module_info["sig_intern_name"], module_info["sig_intern_bits"]):
            port = getTripleIoPort(indent, sig,bit, ";\n")
            declaration += port
            #print(declaration[-50:])
            #print(declaration)

    return declaration

def GetInstance(module_name, instance_name, parameter, vect1, vect2, indent, port_position=0):
    #######################################################################################
    # This function create a verilog instance 
    #######################################################################################
    if port_position!=0 and (type(port_position)!=list or (type(port_position)==list and  len(port_position)>2)):
        print("Error GetInstance, ports_position should be a list of maximum two element")
        exit(-1)
    ...
    for sig,sigin in zip(vect1, vect2):
        if port_position != 0 and cnt==0:
            instance += indent2 + "// Input ports of "+instance_name+"\n"
        if port_position != 0 and cnt == port_position[0]:
            instance += indent2 + "// Output ports of "+instance_name+"\n"
        if port_position != 0 and len(port_position)==2 and cnt == port_position[1]+port_position[0]:
            instance += indent2 + "// FT ports of "+instance_name+"\n"
        if sig == vect1[-1]:
            instance += indent2 + "." + "{:<23}".format(sig) + "( " + " {:<32}".format(sigin) + " )\n"
        else:
            instance += indent2 + "." + "{:<23}".format(sig) + "( " + " {:<32}".format(sigin) + " ),\n"
        cnt+=1
    instance += indent + ");\n"
...
def GetOrigInstance(module_info, instance_name, indent, ft_param=0, ft_sig_list_noindex=0):
    ########################################################################################
    ...
    # module_info -> dictionary with info about the module of which create the instance
    # instance_name -> name of the instance to create
    # ft_param -> parameter to place in the square brackets
    ########################################################################################
    if ft_param!=0 and type(ft_sig_list_noindex) != list: 
        print("ERROR in GetOrigInstance, ft_sig_list_noindex should be a list if you use ft_param option")
        exit(-1)
    ...
    vect1 += module_info["sig_input_name_ft"] + module_info["sig_output_name_ft"]
    vect2 = []
     
    in_n=len(module_info["sig_input_name"])
    for sig in module_info["sig_input_name"]:
        vect2.append(sig)

    out_n=len(module_info["sig_output_name"])
    for sig in module_info["sig_output_name"]:
        vect2.append(sig)
    if ft_param != 0 :
        for sig in module_info["sig_input_name_ft"]:
            if not sig in ft_sig_list_noindex: 
                vect2.append("{:<20}".format(sig)+"[ "+ ft_param +" ]")
            else:
                vect2.append("{:<20}".format(sig))

        for sig in module_info["sig_output_name_ft"]:
            if not sig in ft_sig_list_noindex:
                vect2.append( "{:<20}".format(sig) +"[ "+ ft_param +" ]")
            else:
                vect2.append("{:<20}".format(sig))
    
    instance = GetInstance(module_name, instance_name, parameter, vect1, vect2, indent, [in_n, out_n])
    return instance
...
def  SetSignalElaborationInstance(block, siglist, inoutpar, after_equal, signal_elab, lineno):
    ...
    for sig in siglist:
        if not sig in sigdict[inoutpar]:
            print("WARNING line %d, signal %s isn't an %s signal of set block %s" %(lineno, sig, inoutpar, block['module']))
...
def CreateFtBlock(cmd_dict, block_name, module_info, out_dir, param_index, ft_sig_list_noindex, indent):
    ############################################################################################
    ...
    param_name=GetParamBase(short_name)
  
    ft_datafile = GetElaboratedTravulog(cmd_dict["template_filename"], {'BLOCK':newmod_info}, block_name, short_name, param_name)
    # Print ft_datafile
    ...
    ft_module_info = GetModuleInfo(out_dir+"/"+block_name+"_ft.sv", newmod_info)
    print(json.dumps(ft_module_info,indent=4))
    #exit(-1)
    instance = GetOrigInstance(ft_module_info, short_name+"_ft", indent, param_index, ft_sig_list_noindex)
...
def ElaborateHiddenTravulog(sv_filename, orig_module_info, ft_dir, template_dict):
    ...
    orig_modname = orig_module_info["module"]
    orig_param_name = GetParamBase(orig_modname)
    orig_param_index = orig_param_name+"_I"
    ...
    while lineno < linemax:
        ...
        if line.split(" ")[0] == HTKEY:
            ...
            if " CREATE_FT_BLOCK " in cmd:
                ...
                ft_instance = CreateFtBlock(create_ft_block_dict[block_name], block_name, orig_module_info, ft_dir, orig_param_index, ["clk","rst_n"], tab)
    ...
    fp = open(ft_dir+"/"+ sv_filename.replace(".sv","_ft.sv").split("/")[-1] , "w")
    fp.write(datafile)
    return datafile
...
def NextHTLine(lines, lineno, key):
    ...
    # lines -> list of lines of a file
    # lineno -> current number of line
    # key -> key that identify hidden travulog line, for example "////"
    n=lineno
    HTKEY=key
    while n< len(lines):
